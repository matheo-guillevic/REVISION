...
:::section id="mt421-intro" eyebrow="Semestre 7" title="MT421-Methode-numerique" summary="Analyse numérique et calcul scientifique : erreurs, arithmétique flottante, résolution non linéaire et intégration numérique des EDO."
:::dashboard
...
:::quicklinks
- [Module 1 : erreurs et arithmétique machine](#mt421-module-1)
- [Module 2 : équations non linéaires](#mt421-module-2)
- [Module 3 : EDO](#mt421-module-3)
- [Synthèse](#mt421-synthese)
:::
...
:::section id="mt421-module-1" eyebrow="Module 1" title="Analyse d'erreurs et arithmétique machine" summary="Erreurs numériques, flottants IEEE 754, arrondis, conditionnement et stabilité algorithmique."
...
Un mot mémoire flottant contient trois champs : un bit de signe $s$, un exposant décalé sur $p$ bits, et une mantisse sur $t$ bits. L'exposant machine $E$ est stocké avec un décalage (*bias*) valant $b^{p-1}$ ($E = e + \text{biais}$) :

| Format | Total bits | Base $b$ | Bits mantisse ($t$) | Bits exposant ($p$) | $e_{\min}$ | $e_{\max}$ |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **IEEE single** | 32 | 2 | 23 | 8 | $-128$ | $+127$ |
| **IEEE double** | 64 | 2 | 55 | 8 | $-128$ | $+127$ |
| **IEEE extended** | 80 | 2 | 63 | 15 | $-2^{14}$ | $2^{14}-1$ |

* En simple précision (32 bits), l'exposant est biaisé de $2^7 = 128$.
* **Zone d'Underflow** : $|x| < b^{e_m - 1}$ (le nombre est trop proche de 0 pour être normalisé).
* **Zone d'Overflow** : $|x| \ge b^{e_M}$ (dépassement supérieur de capacité).
:::
...
:::plotly id="mt421-plot-flottants" title="Virgule fixe et virgule flottante" label="Graphique interactif" height="420" caption="Zoome autour de zero : la virgule flottante concentre les valeurs representables pres de zero, alors que la virgule fixe garde un pas constant."
{
  "series": [
    {
      "generator": "function",
...
2. **Arrondi au plus proche (*Rounding*)** :
   $$x_r = \text{sign}(x) \cdot \text{trunc}\left(|x| b^{t-e} + \frac{b^{-1}}{2}\right) \cdot b^{e-t}$$
...
* Pour la **multiplication**, un chiffre de garde ($q \ge 1$) est requis sous peine de perdre un chiffre significatif complet.
* Pour la **soustraction** $x - y$ ($x > y > 0$, $e_x = e_y + k$) :
  ...
  * Si $k > 1$, il n'y a pas de perte de précision.
* L'arithmétique $\text{FP}(b, t, c, q)$ avec $q \ge 1$ garantit $\text{fl}(x * y) = (x * y)(1 + \rho)$ avec $|\rho| \le h$ pour tout $* \in \{+, -, \times, /\}$.
:::

:::block type="definition" title="Sticky Bit"
Pour éliminer les imprécisions résiduelles en mode chopping, un bit supplémentaire nommé **sticky bit** est positionné à 1 si au moins un bit non nul a été éliminé après le chiffre de garde lors du décalage, et 0 sinon. Lors de $x - y$, si le sticky bit de $y$ est égal à 1, on retranche $b^{-t}$ au résultat avant troncature.
:::
...
:::plotly id="mt421-plot-annihilation" title="Amplification par annihilation numerique" label="Graphique interactif" height="420" caption="Quand deux valeurs deviennent proches, le denominateur x-y devient petit et l erreur relative peut exploser."
{
  "series": [
    {
      "generator": "function",
...
:::section id="mt421-module-2" eyebrow="Module 2" title="Résolution d'équations et systèmes non linéaires" summary="Point fixe, accélération de convergence, sécante, Newton-Raphson et systèmes non linéaires."
...
1. **Caractère de Cauchy** : Pour $m > n$, $|x_m - x_n| \le \sum_{j=n}^{m-1} |x_{j+1} - x_j| \le \left(\sum_{j=n}^{m-1} L^j\right) |x_1 - x_0| \le \frac{L^n - L^m}{1 - L} |x_1 - x_0| \xrightarrow[n, m \to \infty]{} 0$.
2. **Complétude** : $\mathbb{R}$ étant complet et $I$ fermé, $x_n \to s \in I$. La continuité de $f$ assure $s = f(s)$.
3. **Unicité** : Si $s_1 = f(s_1)$ et $s_2 = f(s_2)$, alors $|s_1 - s_2| \le L |s_1 - s_2| < |s_1 - s_2|$, ce qui est absurde dès que $s_1 \neq s_2$.


:::plotly id="mt421-plot-point-fixe" title="Iteration de point fixe" label="Graphique interactif" height="420" caption="Les marches montrent la suite x_{n+1}=g(x_n). Ici g est contractante : la suite converge vers l intersection avec y=x."
{
  "series": [
    {
      "generator": "function",
...
:::plotly id="mt421-plot-convergence" title="Comparaison des vitesses de convergence" label="Graphique interactif" height="420" caption="Passe l axe des ordonnees en log : une pente plus forte correspond a une convergence plus rapide."
{
  "series": [
    {
      "generator": "sequence",
...
:::section id="mt421-module-3" eyebrow="Module 3" title="Résolution numérique des équations différentielles ordinaires (EDO)" summary="Problème de Cauchy, méthodes à un pas, multi-pas, stabilité absolue et problèmes raides."
...
:::plotly id="mt421-plot-milne" title="Mode physique et mode parasite de Milne-Simpson" label="Graphique interactif" height="420" caption="Le mode physique decroit, mais le mode parasite alterne de signe et grandit : c est le piege pratique du schema."
{
  "series": [
    {
      "generator": "function",
...
:::plotly id="mt421-plot-stabilite" title="Domaines de stabilite absolue" label="Graphique interactif" height="420" caption="Euler explicite n est stable que dans le disque centre en -1. Les methodes A-stables couvrent tout le demi-plan gauche."
{
  "series": [
    {
...
:::section id="mt421-synthese" eyebrow="Synthèse" title="Synthèse générale des concepts" summary="Tableau récapitulatif des méthodes, atouts et risques numériques."
